...
function App() {
  ...
  const [status, setStatus] = useState('Aguardando envio de documento...');
  ...
  const handleUpload = async (e) => {
    e.preventDefault();
    if (!file) {
      setStatus('Por favor, selecione um arquivo (PDF ou Imagem) primeiro.');
      return;
    }
    ...
    // Simulando o gatilho de upload para o S3
    setTimeout(() => {
      setLoading(false);
      setStatus('✅ Upload concluído! O S3 e o EventBridge já acionaram o processamento via IA (Textract + Bedrock).');
      setFile(null);
    }, 2500);
  };
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col justify-between font-sans">
      ...
      <header className="border-b border-slate-800 bg-slate-950/50 backdrop-blur px-6 py-4 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div className="bg-amber-500 text-slate-950 p-2 rounded-lg font-black text-xl tracking-wider">TR</div>
          <div>
        ...
        <div className="bg-slate-950 border border-slate-800 w-full rounded-2xl p-8 shadow-2xl transition-all">
          <form onSubmit={handleUpload} className="space-y-6">
            ...
            <div className="border-2 border-dashed border-slate-700 hover:border-amber-500 rounded-xl p-8 text-center cursor-pointer bg-slate-900/50 transition-colors group relative">
              {/* Alterado para aceitar PDF e Imagens (PNG, JPG, JPEG) */}
              <input 
                type="file" 
                accept=".pdf, image/*" 
              ...
              <div className="space-y-3">
                ...
                <div className="text-sm font-medium text-slate-300">
                  <span className="text-amber-400 group-hover:underline">Clique para anexar</span> ou arraste o arquivo aqui
                </div>
                <p className="text-xs text-slate-500">Formatos aceitos: PDF ou Imagens (PNG, JPG)</p>
              </div>
            </div>
      ...
      <footer className="border-t border-slate-800 bg-slate-950/20 px-6 py-4 text-center text-xs text-slate-500">
        Hackathon EdN 2026 • Trai Serverless Ingestion Platform
      </footer>
    </div>
  );
}
